...
\begin{tikzpicture}[
  ...
  \node[block, fill=vcolor!20, draw=vcolor] (V) {\textbf{V}\\\scriptsize FSQ tokenizer};
  ...
  \draw[arr] (M) -- node[above, lbl] {$\mathbf{z}_t,\,\mathbf{h}_t$} node[below, lbl] {\tiny 8$\times$8,\;384} (C);
  \draw[arr] (C) -- (output);

  \draw[arr] (C.south) -- ++(0,-0.6) -| node[below, lbl, pos=0.25] {$a_t$} (M.south);

  \path (input.west) -- (output.east) coordinate[midway] (mid);
  \node[below=1.75cm of mid, lbl] (legend) {%
    $\mathbf{x}_t \in \mathbb{R}^{64 \times 64}$: Sobel frame \quad
    $\mathbf{z}_t \in \{0,\ldots,999\}^{64}$: tokens \quad
    $\mathbf{h}_t \in \mathbb{R}^{384}$: hidden state \quad
