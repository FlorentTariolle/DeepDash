...
\begin{enumerate}
  \item an end-to-end discrete world-model agent for pixel-based control of the original, non-paused Geometry Dash game, with a decoder-free capture-to-action path that sustains a 60-FPS live cadence on a consumer GPU; and
...
DashVMC instead uses a discrete world model to optimize a policy in imagination and closes the control loop through the original non-paused game on consumer hardware.
...
Counting tied weights once, the encoder, decoder, complete training-time dynamics model, and controller contain \(16{,}652{,}911\) parameters in total.
...
We use Generalized Advantage Estimation (GAE) with \(\gamma=0.995\), \(\lambda=0.95\), PPO clip 0.2, and auxiliary-action loss weight 0.1.
...
The expert subset contains less than 20 minutes of gameplay.
...
DashVMC shows that a lightweight discrete world model can support both imagined policy improvement and live reactive control in a timing-sensitive game.
